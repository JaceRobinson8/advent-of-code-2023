...
class SequenceStack:  # a single input line
    ...
    def preprend_stack(self) -> None:
        """Starting at bottom of stack, append one new value in front of each sequence"""
        assert len(self.seqs) >= 2  # must be at least 2 sequences from build_stack()
        new_val = 0  # initial seq appends 0
        # reverse to go bottom up
        for bot_seq, top_seq in pairwise(reversed(self.seqs)):
            bot_seq.insert(0, new_val)
            # first number of current sequence - last number in previous seq
            new_val = top_seq[0] - bot_seq[0]
    ...
    @property
    def top_pre_extrap_val(self) -> int:
        """Return last value of first sequence"""
        return self.seqs[0][0]
...
def parse_input(file_path: Path = Path("./input/input.txt")) -> list[SequenceStack]:
    ...
    with open(file_path, "r") as file:
        ...
        [stack.preprend_stack() for stack in stacks]
        logger.debug(stacks)
